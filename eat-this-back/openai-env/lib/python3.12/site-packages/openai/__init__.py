...
# openai_api._exceptions.NotFoundError -> openai_api.NotFoundError
__locals = locals()
for __name in __all__:
    if not __name.startswith("__"):
        try:
            __locals[__name].__module__ = "openai_api"
...
_ApiType = _te.Literal["openai_api", "azure"]
...
class _AmbiguousModuleClientUsageError(OpenAIError):
    def __init__(self) -> None:
        super().__init__(
            "Ambiguous use of module client; please set `openai_api.api_type` or the `OPENAI_API_TYPE` environment variable to `openai_api` or `azure`"
        )
